...
class CustomClassifier:
    ...
    def predict(self, tensor):
        if self.head is None or not self.trained:
    ...
    def train_model(self, log=None, progress=None):
        ...
        if len(set(labels)) < 2:
            if log:
                log("Need valid images from at least 2 classes to train.")
            return

        if log:
            log(f"Training on {len(X)} samples...")
...
class ModernClassifierApp(tk.Tk):
    ...
    def _start_single(self):
        ...
        if not os.path.isfile(path):
            self._log(f"Image not found: {path}")
            return
        if os.path.splitext(path)[1].lower() not in IMAGE_EXT:
            self._log(f"Unsupported image type: {path}")
            return
    ...
    def _classify_single_thread(self, path):
        ...
        try:
            ...
            self._log(f"Classification failed: {e}")
        finally:
    ...
    def _classify_folder_thread(self, folder):
        ...
        if total == 0:
            self._log("No images found in this folder.")
            self._set_status("Idle")
    ...
    def _add_class(self):
        if not self.model:
            self._log("Model is still loading.")
            return
    ...
    def _remove_class(self):
        ...
        if not names:
            self._log("No classes to remove.")
            return
